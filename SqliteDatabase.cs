...
public class SqliteDatabase
{
	...
	public SqliteDatabase(string dbName){
		...
		pathDB = Application.persistentDataPath +"/"+ dbName;
		...
		if (!System.IO.File.Exists(pathDB)){
			//original path
			string sourcePath = Application.streamingAssetsPath+"/"+ dbName;
			
			//validate the existens of the DB in the original folder (folder "streamingAssets")
			if(System.IO.File.Exists(sourcePath)){
				//copy file
				System.IO.File.Copy(sourcePath,pathDB);
			}else{
				CanExQuery = false;
    ...
    {
		if(!CanExQuery) {Debug.Log("ERROR: Can't execute the query, verify DB origin file"); return;}
    ...
    {
		if(!CanExQuery) {Debug.Log("ERROR: Can't execute the query, verify DB origin file"); return null;}
